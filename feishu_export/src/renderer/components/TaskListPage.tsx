...
const TaskListPage: React.FC<TaskListPageProps> = ({ onNewTask }) => {
  ...
  const handleDeleteTask = async (taskId: string) => {
    try {
      await window.electronAPI.deleteDownloadTask(taskId);
   ...
   * 打开保存目录
   */
  const handleOpenSaveDir = async (savePath: string) => {
    try {
      await window.electronAPI.openDirectory(savePath);
    } catch (error) {
  ...
  const handleStartDownload = async (taskId: string) => {
    try {
      ...
      await window.electronAPI.executeDownloadTask(taskId, access_token);
  ...
  const handleAutoResumeDownloadTasks = async () => {
    try {
      const result = await window.electronAPI.resumeDownloadTasks();
      if (result.success) {
  ...
  useEffect(() => {
    ...
    const handleAutoResumeTask = async (data: any) => {
      ...
      try {
        ...
        if (access_token) {
          ...
          await window.electronAPI.executeDownloadTask(data.taskId, access_token);
          loadTasks(); // 重新加载任务列表
        }
      } catch (error) {
        console.error('自动恢复下载任务失败:', error);
    ...
    window.electronAPI.onDownloadFileError?.(handleDownloadFileError);
    
    // 注册恢复下载任务监听器
    if (window.electronAPI.onResumeDownloadTasks) {
      window.electronAPI.onResumeDownloadTasks(handleResumeDownloadTasks);
    }
    
    // 注册自动恢复单个任务监听器
    if (window.electronAPI.onAutoResumeTask) {
      window.electronAPI.onAutoResumeTask(handleAutoResumeTask);
    }
  ...
  // 任务表格列定义
   const taskColumns = [
     ...
     },
     {
       ...
       key: 'savePath',
       width: '20%',
       render: (savePath: string) => {
         const maxLength = 20;
         const displayPath = savePath.length > maxLength 
           ? `${savePath.slice(-maxLength)}...` 
           : savePath;
         return (
           <Text 
             ellipsis={{ tooltip: savePath }} 
       ...
       render: (_: any, record: DownloadTask) => (
         <Space>
           ...
           <Button
             ...
             size="small"
             onClick={() => handleOpenSaveDir(record.savePath)}
             title="打开保存目录"
           />
           <Popconfirm
             title="确定要删除这个任务吗？"
    ...
    },
    {
       ...
       render: (status: string, record: DownloadFile) => {
         ...
         if (status === 'failed' && record.errorMessage) {
           return (
             <Tooltip title={record.errorMessage} placement="top">
               {statusTag}
             </Tooltip>
           );
         }
  ...
  const hasFailedFiles = (task: DownloadTask): boolean => {
    return task.files.some(file => file.status === 'failed');
  };
  ...
  const handleRetryFile = async (taskId: string, fileToken: string) => {
    try {
      ...
      await window.electronAPI.retryDownloadFile(taskId, fileToken, access_token);
  ...
  const handleRetryFailedFiles = async (taskId: string) => {
    try {
      ...
      for (const file of failedFiles) {
        try {
          await window.electronAPI.retryDownloadFile(taskId, file.token, access_token);
        } catch (error) {
  ...
  const expandedRowRender = (record: DownloadTask) => {
    // 为文件数据添加taskId属性
    const filesWithTaskId = record.files.map(file => ({ ...file, taskId: record.id }));
